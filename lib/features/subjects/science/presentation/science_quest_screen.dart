...
class _ScienceQuestScreenState extends ConsumerState<ScienceQuestScreen> {
  ...
  Future<void> _loadQuestions() async {
    ...
    try {
      final res = await http.post(
        ...
        body: jsonEncode({'subject': 'Science', 'grade': 'Grade 8'}),
    ...
    Future.delayed(const Duration(milliseconds: 900), () {
      ...
      if (_current < _questions.length - 1) {
        ...
        ref.read(gamificationProvider.notifier)
            .updateSubjectProgress('Science', _score, _questions.length);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Science Quest"), backgroundColor: color, foregroundColor: Colors.white),
      body: Padding(
  ...
  Widget _buildFinished(Color color) => Scaffold(
    appBar: AppBar(title: const Text("Science"), backgroundColor: color, foregroundColor: Colors.white),
    body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Icon(Icons.celebration, size: 80, color: Color(0xFF534AB7)),
      const SizedBox(height: 16),
