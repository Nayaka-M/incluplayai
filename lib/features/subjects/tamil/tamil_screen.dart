...
class _TamilScreenState extends ConsumerState<TamilScreen> {
  ...
  Future<void> _loadQuestions() async {
    ...
    try {
      final res = await http.post(
        ...
        body: jsonEncode({'subject': 'Tamil Language and Literature', 'grade': 'Grade 8'}),
    ...
    Future.delayed(const Duration(milliseconds: 900), () {
      ...
      if (_current < _questions.length - 1) {
        ...
        ref.read(gamificationProvider.notifier)
            .updateSubjectProgress('Tamil', _score, _questions.length);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Tamil Quest"), backgroundColor: color, foregroundColor: Colors.white),
      body: Padding(
  ...
  Widget _buildFinished(Color color) => Scaffold(
    appBar: AppBar(title: const Text("Tamil"), backgroundColor: color, foregroundColor: Colors.white),
    body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Icon(Icons.celebration, size: 80, color: Color(0xFFBA7517)),
      const SizedBox(height: 16),
