...
class _MathQuestScreenState extends ConsumerState<MathQuestScreen> {
  ...
  Future<void> _loadQuestions() async {
    ...
    try {
      final res = await http.post(
        ...
        body: jsonEncode({'subject': 'Mathematics', 'grade': 'Grade 8'}),
    ...
    Future.delayed(const Duration(milliseconds: 900), () {
      ...
      if (_current < _questions.length - 1) {
        ...
        ref.read(gamificationProvider.notifier)
            .updateSubjectProgress('Mathematics', _score, _questions.length);
  ...
  Widget build(BuildContext context) {
    const color = Color(0xFF185FA5);
    if (_loading) return _buildLoading(color, 'Mathematics');
    if (_error != null) return _buildError(color, 'Mathematics');
    if (_finished) return _buildFinished(color, 'Mathematics');

    final q = _questions[_current];
    return Scaffold(
      appBar: AppBar(title: const Text("Math Quest"), backgroundColor: color, foregroundColor: Colors.white),
      body: Padding(
  ...
  Widget _buildLoading(Color color, String subject) => Scaffold(
    appBar: AppBar(title: Text(subject), backgroundColor: color, foregroundColor: Colors.white),
    body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      CircularProgressIndicator(color: color),
      const SizedBox(height: 20),
      const Text("Generating AI questions...", style: TextStyle(fontSize: 16)),
      const SizedBox(height: 8),
      Text("Powered by Groq + NCERT syllabus",
          style: TextStyle(fontSize: 13, color: Colors.grey[600])),
    ])),
  );

  Widget _buildError(Color color, String subject) => Scaffold(
    appBar: AppBar(title: Text(subject), backgroundColor: color, foregroundColor: Colors.white),
    body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
  ...
  Widget _buildFinished(Color color, String subject) => Scaffold(
    appBar: AppBar(title: Text(subject), backgroundColor: color, foregroundColor: Colors.white),
    body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Icon(Icons.celebration, size: 80, color: Color(0xFF185FA5)),
      const SizedBox(height: 16),
      Text("Score: $_score / ${_questions.length}",
          style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold)),
      Text("+${_score * 10} points earned!",
